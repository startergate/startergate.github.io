  ...
  <div className="overlay-item project-detail" id={'overlay-' + data.id}>
    <div className="project-detail-title">
      ...
      <div className={'project-detail-time'}>
        ...
        <span className={`project-detail-time-elapsed`}>{data.finishedAt ? `${(new Date(data.finishedAt).getTime() - new Date(data.startedAt).getTime()) / 86400000}일` : "　"}</span>
        <span className={`project-detail-time-end`}>{data.finishedAt ? getKoreanDateString(new Date(data.finishedAt)) : "개발 중"}</span>
      </div>
      <span
